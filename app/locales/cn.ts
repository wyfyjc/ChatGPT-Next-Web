...
const cn = {
  ...
  Settings: {
    ...
    Sync: {
      ...
      Overview: (overview: any) => {
        return `${overview.chat} 次对话，${overview.message} 条消息，${overview.prompt} 条预设提示词，${overview.mask} 个面具`;
      },
      ImportFailed: "导入失败",
    },
    Mask: {
    ...
    CompressThreshold: {
      Title: "历史消息长度压缩阈值",
      SubTitle: "当未压缩的历史消息长度超过该值时，将进行压缩",
    },
  ...
  Store: {
    ...
    Prompt: {
      ...
      Topic:
        "使用十个字以内简明扼要地总结这段对话的主题，不要解释、不要标点、不要语气词、不要多余文本，不要加粗，如果没有明确的主题，请直接返回“闲聊”",
      Summarize:
